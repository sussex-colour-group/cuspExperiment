...
RGBs=Feb20_2025_160_primaries(:,2:4);
...
textHeights = linspace((-4/10)*screenHeight,(4/10)*screenHeight,7);
crsDrawString([0 textHeights(1)],'Welcome to this perception experiment!');
crsDrawString([0 textHeights(2)],'You will be given 5 practice trials before we begin.');
crsDrawString([0 textHeights(3)],'Fixate your eyes on the dot in the middle of');
crsDrawString([0 textHeights(4)],'the screen and watch out for any flashes you see.');
crsDrawString([0 textHeights(5)],'Then, record what you saw using the taped' );
crsDrawString([0 textHeights(6)],'keys. Press space to confirm your guess.' );
crsDrawString([0 textHeights(7)],'Are you ready? Press any ARROW key to start' );
...
sessionTime=0;
k=0;
while k <= 5 % Runs 5 practice trials
    k=k+1;
    CurrentTime=GetSecs;
...
crsSetDrawPage(CRS.VIDEOPAGE,11,1);
textHeights = linspace((-1/10)*screenHeight,(1/10)*screenHeight,3);
crsDrawString([0 textHeights(1)],'This is the end of the practice block.');
crsDrawString([0 textHeights(2)],'Please let the experimenter know');
crsDrawString([0 textHeights(3)],'when you are ready to start.');
